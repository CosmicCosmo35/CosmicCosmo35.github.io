...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($title && $body && !$error && strlen($title) <= MAX_TITLE_LENGTH && strlen($body) <= MAX_BODY_LENGTH) {
    ...
    header("Location: /science_post/$id");
    exit;
  }
}
?>
...
<body>
  ...
  <div class="content">
    ...
    <?php endif; ?>
    <?php if ($_SERVER['REQUEST_METHOD'] === 'GET' || !$title): ?>
    <form method="post" class="reply-form" enctype="multipart/form-data">
      ...
      <p class="meta" style="margin-top:-8px">After posting, there is a <?= POST_DELAY ?> second wait.</p>
